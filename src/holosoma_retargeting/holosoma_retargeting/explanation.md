...
2. urdf , xml , obj格式文件及相关概念

    * URDF：主要用于描述单个机器人的运动学结构（树状结构，只有一个 Root）。

    * XML：专为物理仿真Mujoco设计。它可以定义整个“世界”，包括多个独立的物体、复杂的碰撞、真实的光照和材质。

    * OBJ: 纯几何网格文件（顶点+面片），不含物理属性、不含世界位置。

    * mesh/box/sphere/cylinder: 资产注册
...
4. 硬约束和软约束：
    ...
    硬约束： 约束添加在如 SQP（序列二次规划）优化器的约束列表 constraints 中，比如holosoma的碰撞检测
...
5. 使用新输入数据格式时（非原开源项目支持），retarget得到的机器人会躺在地面：

    原因：Y‑up vs Z‑up
    Y‑up：垂直方向是 Y 轴（常见于一些 mocap/BVH 数据格式）。
    Z‑up：垂直方向是 Z 轴（MuJoCo 默认就是 Z‑up）。
...
8. wrist约束和cube约束开始帧的动作突变明显：

    （1）权重变化函数改为五次函数：五次多项式在 $t=0$（开始过渡）和 $t=1$（结束过渡）处不仅一阶导数（速度）为 0，二阶导数（加速度）也为 0。

    （2）平滑修正目标：不要要求机器人“一步到位”修正所有姿态误差，而是要求它在当前帧只修正 alpha * 原始误差。

    （3）拓扑结构恒定化：关于球杆约束的三个点，始终存在于网格中，只是在不需要它们的时候，将它们的权重设为 0。

    （4）球杆相关三个虚拟点偏移量平滑：同步修改人类侧（目标）和机器人侧（执行）的计算逻辑，让球杆虚拟点随 alpha 动态“生长”。
    
9. 之前那个data.type改之后导致机器人蜷缩扭曲的bug


10. 让推杆动作比较稳定，杆不会乱晃：

    三个虚拟点（左手搭杆点，右手握杆点，杆尖）在拉普拉斯网络中的顶点权重显著更高。

    在优化问题中，拉普拉斯项的代价函数通常表示为：
    $$\text{Cost}{lap} = \sum{i=1}^{V} w_{v,i} \| (L \cdot \mathbf{v})i - \mathbf{\delta}_i \|^2$$
    其中：
    $(L \cdot \mathbf{v})i$ 是顶点 $i$ 当前的拉普拉斯坐标（代表其与邻居的相对几何关系）。
